 ...
 * the transform.
 * later, after locking the transform, it can combine the local aggregation result
...
/* WindowResultT: one window's agg result. Not exposing to downstream transforms.
 *
...
//	using AggResultT = unordered_map<Window, WindowResultT, Window>; /* need a hash func */
...
//  const long partial_results_grow_ = 4 * 1024 * 1024 / sizeof (WindowResultT);
  ...
  bool is_start_win_assigned(void) {
	...
	void clear_start_win(void) {
		...
		this->start_win_.start =
				boost::posix_time::time_duration(0,0,0);
		this->start_win_.duration =
				boost::posix_time::time_duration(0,0,0);
	}
...
//		return index;
		...
		long index = (win.start - start).total_milliseconds()
							/ this->win_duration_ms_;
...
public:
  StatefulTransform(string name)
	: PTransform(name), unflushed_(0) {
...
//						start_win_.start = win.start - seconds(10);
//						start_win_.duration = win.duration;

						start_win_ = Window(
								wm_rounddown_epoch(win.window_start() - seconds(10), win.duration),
										win.duration);
...
//							partial_results_.resize(s + delta, acc);
							...
							for (int k = s; k < s + delta; k++) { /* init each window result explicitly */
								TransformT::aggregate_init(&partial_results_[k]);
							}
						}
					} else {
			...
			TransformT::combine(partial_results_[index], res); /* this has to be MT safe */
...
restart:
  	...
  	for (int i = unflushed; i <= ret_index; i++) {
  		...
  		Window w(start_win_.window_start() + milliseconds(i * win_duration_ms_),
  				start_win_.duration);
//  		ret[w] = partial_results_[i]; /* copy */
  ...
  ptime RetrieveStateSliding(AggResultT* out, ptime wm_up, int multi)
  {
  	...
  	xzl_assert(t > ptime(min_date_time) + milliseconds(multi * this->win_duration_ms_)
  ...
  ptime RetrieveState(AggResultT* out,
  		bool purge = false, ptime wm = max_date_time, int n = -1)
  {
  	auto & ret = *out;
...
restart:
  	...
  	if (wm == max_date_time)
  		...
  		cnt = (wm - start_win_.window_start()).total_milliseconds()
  				/ start_win_.duration.total_milliseconds() - unflushed;
  		/* e.g.
...
//  	cout << "XXXX  cnt: " << cnt << "win duration: " start_win_.duration.total_milliseconds() << endl;
...
//  		ret[w] = partial_results_[i]; /* copy */
  	...
  	if (unflushed == s) {
  		 ...
  		 * */
  		ret_min_ts = start_win_.window_start()
  				+ milliseconds(unflushed * start_win_.duration.total_milliseconds());
  	} else {
  		...
  		rlock.unlock();
  		{
				...
				if (unflushed_ > partial_results_gc_) {
					...
					if (unflushed_ == s) { /* empty internal state */
						...
						start_win_.start
							+= milliseconds(unflushed_ * start_win_.duration.total_milliseconds());
					}
